...
function walk(dir) {
  for (const item of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (item.isDirectory()) walk(p);
      ...
      // On utilise le path complet normalise
      const norm = p.split(path.sep).join('/');
      // Match /pages/api/X.ts ou /pages/api/X/Y.ts
      const m = norm.match(/^src\/pages\/(api|auth)\/(.+)\.ts$/);
      let rel = '';
      if (m) rel = '/' + m[1] + '/' + m[2];
...
console.log('Has /api/user-appointments/[id]:', '/api/user-appointments/[id]' in apis);
...
for (const c of checks) {
  ...
  // Pas de \\b, on matche 'export const NEED' (simple)
  const idx = src.indexOf('export const ' + c.need);
  if (idx >= 0) {
    // Verifier que c\\'est bien suivi d\\'un delimiteur (espace, :, =)
    const after = src.substring(idx + 'export const '.length + c.need.length, idx + 'export const '.length + c.need.length + 5);
    if (/^[\s:=]/.test(after)) { ok++; }
    else { missing.push(c.url + ' (PARTIAL MATCH: ' + after + ')'); broken++; }
  } else {
    missing.push(c.url + ' (NO ' + c.need + ')'); broken++;
  }
}
