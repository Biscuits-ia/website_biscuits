...
function walk(dir) {
  for (const item of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (item.isDirectory()) walk(p);
      ...
      // Stocker la cle comme /api/foo ou /auth/foo
...
console.log('Total routes chargees:', Object.keys(apis).length);

function findApi(url) {
  // /api/contact -> /api/contact
  // /api/user-appointments/[id] -> /api/user-appointments (routes dynamiques = la racine)
  if (url.includes('/[')) {
    return apis[url.split('/[')[0]];
  }
  return apis[url];
}
...
for (const c of checks) {
  ...
  if (!src) { console.log('MISSING FILE: ' + c.url); broken++; continue; }
  const re = new RegExp('export const ' + c.need + '\\b');
  if (!re.test(src)) { missingMethods.push(c.url + ' (' + c.need + ')'); broken++; }
  else { ok++; }
}
console.log('OK : ' + ok);
...
if (missingMethods.length) {
  console.log('\\nDetail KO:');
  for (const m of missingMethods) console.log('  - ' + m);
}
