...
function findApi(url) {
  // Convertir en chemin relatif : /api/demandes/creer -> demandes/creer.ts
  if (url.startsWith('/api/')) {
    const tail = url.slice(5); // demandes/creer ou user-appointments/[id]
    return Object.keys(apis).find(k => {
      const norm = k.replace(/\\/g, '/');
      return norm.endsWith('/api/' + tail + '.ts');
    });
  } else if (url.startsWith('/auth/')) {
    const tail = url.slice(6);
    return Object.keys(apis).find(k => k.endsWith('/auth/' + tail + '.ts'));
  }
}
