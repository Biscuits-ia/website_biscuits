...
// Normaliser les cles en forward slashes
const apis = {};
function walk(dir) {
  for (const item of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (item.isDirectory()) walk(p);
      ...
      apis[norm] = fs.readFileSync(p, 'utf8');
    }
  }
}
walk('src/pages/api');
...
function findApi(url) {
  if (url.startsWith('/api/')) {
    ...
    return Object.keys(apis).find(k => k.endsWith('/api/' + base));
  } else if (url.startsWith('/auth/')) {
    const base = url.slice(6) + '.ts';
    return Object.keys(apis).find(k => k.endsWith('/auth/' + base));
  }
}
