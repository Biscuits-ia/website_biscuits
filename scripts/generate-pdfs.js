...
const PRIMARY_LIGHT  = '#b6734d';  // --color-primary-light  Caramel
const SECONDARY      = '#a06e3f';  // --color-secondary  Noisette
const CAPPUCCINO     = '#c4804a';  // --color-first  Cappuccino
const LATTE          = '#cea26a';  // --color-second  Latte
const MOUSSE         = '#dcc188';  // --color-third  Mousse crème
...
const BG             = '#f7eed8';  // --color-bg
const BG_ELEVATED    = '#faf5e6';  // --color-bg-elevated
...
function sectionTitle(doc, text) {
  ...
  doc.fontSize(12).fillColor(PRIMARY_DARK).font('Helvetica-Bold').text(text, 62, doc.y + 6);
  doc.moveDown(0.8);
}
...
function checkItem(doc, text, checked = false) {
  ...
  doc.rect(x, y + 1, 10, 10).lineWidth(1.5).strokeColor(BORDER).stroke();
  if (checked) {
    doc.fontSize(9).fillColor(PRIMARY).text('✓', x + 1, y);
  }
  doc.fontSize(10).fillColor(TEXT).font('Helvetica').text(text, x + 16, y, { width: 470 });
  doc.moveDown(0.45);
}
...
function bulletItem(doc, text, indent = 0) {
  ...
  doc.fontSize(10).fillColor(TEXT).font('Helvetica').text(text, x + 10, doc.y, { width: 470 - indent });
  doc.moveDown(0.4);
}
...
function warningBox(doc, text) {
  ...
  doc.fontSize(9.5).fillColor(PRIMARY_DARK).font('Helvetica-Bold').text('⚠  ' + text, 62, y + 9, { width: 470 });
  doc.moveDown(1.2);
}
...
function infoBox(doc, text) {
  ...
  doc.fontSize(9.5).fillColor(PRIMARY_DARK).font('Helvetica').text(text, 62, y + 9, { width: 470 });
  doc.moveDown(1.4);
}

function bodyText(doc, text) {
  doc.fontSize(10).fillColor(TEXT).font('Helvetica').text(text, 58, doc.y, { width: 490, align: 'justify' });
  doc.moveDown(0.5);
}
...
function generateChecklistAntiArnaque() {
  ...
  header(doc, 'Checklist Anti-Arnaque Numérique', 'Protégez votre association des fraudes en ligne — Biscuits IA 2026');

  sectionTitle(doc, '1. Arnaque au virement (fraude au président)');
  bodyText(doc, 'Ce type de fraude cible les trésoriers et directeurs financiers. Un fraudeur se fait passer pour un dirigeant et demande un virement urgent et confidentiel.');
  ...
  bodyText(doc, 'Des e-mails imitant des organismes officiels (CAF, URSSAF, banques, Impôts) tentent de voler vos identifiants ou de vous faire télécharger un malware.');
  checkItem(doc, 'Vérifier l\'adresse e-mail complète de l\'expéditeur (pas seulement le nom affiché)');
  checkItem(doc, 'Ne jamais cliquer sur un lien dans un e-mail inattendu — aller directement sur le site officiel');
  ...
  checkItem(doc, 'Utiliser un gestionnaire de mots de passe (Bitwarden, KeePass) pour des mots de passe uniques');
  ...
  checkItem(doc, 'Mettre en place un registre des fournisseurs habituels et alerter en cas de nouveau RIB');
  ...
  checkItem(doc, 'Vérifier que votre association est bien référencée sur le répertoire officiel data.gouv.fr');
  ...
  checkItem(doc, 'Activer la double authentification sur : e-mail, réseaux sociaux, site web, banque');
  ...
  infoBox(doc, 'En cas de fraude avérée : portez plainte au commissariat, contactez votre banque dans l\'heure et signalez sur cybermalveillance.gouv.fr');
...
function generateGuidePhishing() {
  ...
  header(doc, 'Guide Anti-Phishing pour Associations', 'Reconnaître, éviter et réagir face aux tentatives d\'hameçonnage — Biscuits IA 2026');

  sectionTitle(doc, 'Qu\'est-ce que le phishing ?');
  bodyText(doc, 'Le phishing (hameçonnage) est une technique de cyberattaque où des fraudeurs se font passer pour une entité de confiance (banque, administration, plateforme connue) afin de voler des informations sensibles : identifiants, coordonnées bancaires, données personnelles.');
  bodyText(doc, 'Les associations sont des cibles de choix car elles gèrent des données de bénéficiaires, des financements publics et ont souvent des équipes bénévoles moins formées à la cybersécurité.');

  sectionTitle(doc, '1. Reconnaître un e-mail de phishing');
  bulletItem(doc, 'L\'adresse expéditeur ne correspond pas au domaine officiel (ex: contact@impots-remboursement.com)');
  ...
  warningBox(doc, 'Vérifiez TOUJOURS l\'URL en survolant le lien avant de cliquer. Ne faites pas confiance au nom affiché.');
  ...
  checkItem(doc, 'Si données bancaires saisies : appeler votre banque en urgence et bloquer la carte');
  ...
  infoBox(doc, 'Ressource officielle : cybermalveillance.gouv.fr — Assistance gratuite aux particuliers et associations victimes de cybercriminalité.');

  sectionTitle(doc, '5. Configuration technique recommandée');
  bulletItem(doc, 'SPF : enregistrement DNS qui liste les serveurs autorisés à envoyer en votre nom');
  bulletItem(doc, 'DKIM : signature cryptographique ajoutée à chaque e-mail sortant');
  bulletItem(doc, 'DMARC : politique déclarant ce que les destinataires doivent faire en cas d\'échec SPF/DKIM');
...
function generateChecklistRGPD() {
  ...
  header(doc, 'Checklist RGPD pour Associations', 'Mise en conformité Règlement Général sur la Protection des Données — Biscuits IA 2026');

  bodyText(doc, 'Le RGPD s\'applique à TOUTE association qui collecte ou traite des données personnelles de personnes physiques dans l\'UE. Une mise en conformité n\'est pas optionnelle : les amendes peuvent atteindre 2 % du budget annuel.');
  ...
  checkItem(doc, 'Identifier toutes les données personnelles collectées (noms, e-mails, téléphones, santé…)');
  checkItem(doc, 'Documenter la finalité de chaque traitement (pourquoi collectez-vous ces données ?)');
  checkItem(doc, 'Identifier la base légale de chaque traitement (consentement, contrat, intérêt légitime…)');
  checkItem(doc, 'Recenser tous les sous-traitants qui accèdent à vos données (hébergeur, CRM, prestataire…)');
  checkItem(doc, 'Définir la durée de conservation pour chaque catégorie de données');

  sectionTitle(doc, '2. Droits des personnes');
  checkItem(doc, 'Informer les personnes lors de la collecte (politique de confidentialité accessible)');
  ...
  warningBox(doc, 'Le consentement doit être libre, éclairé, spécifique et univoque. Une case pré-cochée est INVALIDE.');

  sectionTitle(doc, '3. Sécurité des données');
  checkItem(doc, 'Chiffrer les bases de données contenant des données personnelles');
  checkItem(doc, 'Sauvegarder les données régulièrement (règle 3-2-1 : 3 copies, 2 supports, 1 hors site)');
  ...
  checkItem(doc, 'Protéger les échanges d\'e-mails contenant des données personnelles (chiffrement)');
  ...
  checkItem(doc, 'Ne transférer des données hors UE que vers des pays adéquats ou avec des garanties (SCC)');
  ...
  infoBox(doc, 'Ressource officielle : cnil.fr/fr/les-outils-de-la-conformite — La CNIL propose des modèles de registre et de politique de confidentialité gratuits adaptés aux associations.');
...
function generateOutilsOpenSource() {
  ...
  header(doc, 'Outils Open Source pour Associations', 'Sélection 2026 — Outils gratuits et respectueux des données — Biscuits IA');

  bodyText(doc, 'Ce guide recense les meilleurs outils open source et gratuits pour les associations en 2026. Tous respectent le RGPD et peuvent être auto-hébergés ou utilisés en SaaS. Une alternative éthique et souveraine à chaque outil propriétaire.');

  sectionTitle(doc, '1. Communication interne & messagerie');
  bulletItem(doc, 'Element / Matrix — Messagerie chiffrée de bout en bout, alternative à Slack/Teams');
  ...
  bulletItem(doc, 'Nextcloud — Suite bureautique complète (fichiers, agenda, visio, notes, tableur)');
  ...
  bulletItem(doc, 'Astro — Générateur de sites statiques ultra-performant (utilisé par Biscuits IA !)');
  ...
  bulletItem(doc, '  → Gestion des adhérents, dons, événements, envois d\'e-mails, compatible WordPress', 14);
  ...
  bulletItem(doc, 'Jitsi Meet — Visioconférence sans compte, chiffrée, hébergée par Framasoft (Framatalk)');
  bulletItem(doc, 'BigBlueButton — Plateforme d\'enseignement en ligne avec tableau blanc, quiz, sondages');
  ...
  infoBox(doc, 'Conseil Biscuits IA : commencez par Nextcloud (fichiers + agenda + visio) et Bitwarden (mots de passe). Ces deux outils couvrent 80 % des besoins numériques d\'une association et peuvent être configurés en une journée.');
